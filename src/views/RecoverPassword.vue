...
const recoverPassword = () => {
  axios
    .post('https://api-users-smoky.vercel.app/recoverpassword', {
      email: form.email,
    })
    .then((res) => {
      console.log(res)
      if (res.data.sucess) {
        localStorage.setItem('token-recover', res.data.token)
  ...
  <transition name="fade">
    <div class="login-wrapper">
      <div class="login-card">
        ...
        <h1 class="title is-3 has-text-centered mb-3">Recuperação Senha</h1>
        <p class="subtitle is-6 has-text-centered mb-4">Entre com o email para recuperar senha</p>
        ...
        <!-- BOTÃO -->
        <button class="button is-fullwidth mt-2 custom-button" @click="recoverPassword">
          Entrar
        </button>
      </div>
    </div>
